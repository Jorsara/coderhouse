...
const persistenciaMemory = class persistenciaMemory {
  ...
  // Buscar por nombre
  buscarNombre = async (req, res, next) => {
    try {
      // Buscar el producto y mostrarlo
  ...
  buscarCodigo = async (req, res, next) => {
    try {
      ...
      const result = json.filter(
        (product) => product.codigo == req.query.codigo
      );
  ...
  // Buscar por precio
  buscarPrecio = async (req, res, next) => {
    try {
      // Buscar el producto y mostrarlo
      const result = json.filter(
        (product) => product.price == req.query.minimo
      );
  ...
  // Buscar por stock
  buscarStock = async (req, res, next) => {
    try {
      // Buscar el producto y mostrarlo
      const result = json.filter(
        (product) => product.stock == req.query.minimo
      );

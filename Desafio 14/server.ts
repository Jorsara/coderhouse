...
const fs = require('fs');
const mensajes:{message:{value:string, email:string, dateStr:string},id:string}[] = [];
...
io.on('connection', async (socket:any)=>{
    ...
    socket.on('mensaje-nuevo', async(data:{value:string, email:string, dateStr:string, id:string})=>{
        console.log(data);
        data.id = socket.id;
        io.emit("actualizar-chat", data);
